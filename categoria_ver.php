...
$query_DatosProductos = sprintf("SELECT * FROM producto WHERE producto.Categoria=%s", GetSQLValueString($varCategoria_DatosProductos, "int"));
$query_limit_DatosProductos = sprintf("%s LIMIT %d, %d", $query_DatosProductos, $startRow_DatosProductos, $maxRows_DatosProductos);
$DatosProductos = mysql_query($query_limit_DatosProductos, $conexiondeportes) or die(mysql_error());
...
<div class="container">
  ...
  <?php do { ?>
   <div class="producto"><div class="fotoproducto"><img src="documentos/productos/<?php echo $row_DatosProductos['Imagen']; ?>" width="120"" height="120" /></div><div class="textoproducto">
     <a href="ver_producto.php?recordID=<?php echo $row_DatosProductos['idProducto']; ?>"><?php echo $row_DatosProductos['Nombre']; ?></a>. Precio:<?php echo $row_DatosProductos['Precio']; ?></div></div>
    <?php } while ($row_DatosProductos = mysql_fetch_assoc($DatosProductos)); ?>
      ...
      <p>
        <?php if ($totalRows_DatosProductos == 0) { // Show if recordset empty ?>
          Todavía no hay productos de esta categoría.
